...
```text
Read CLAUDE.md, README.md, and docs/ANDROID_BUILD_GUIDE.md. Build the
...
- Set up Hilt application/activity integration, a Material 3 theme, edge-to-edge insets, and navigation placeholders for the five destinations.
...
- Home shows story cards, active sort, reporting-country/source counts, and a demo-data disclosure. Tap a card to open Story by ID.
...
- Settings persists reading language, home country/region, enabled sources, translation display preference, and system/light/dark theme in DataStore.
...
Update README with actual toolchain versions, import/build/run instructions, screenshots if available, and the implemented vs planned feature boundary. Report checks actually executed, any blocked checks, the debug APK path, and remaining work. Keep future ingestion, translation providers, and Lens Gap scoring in the roadmap.
